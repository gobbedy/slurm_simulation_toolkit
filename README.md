 ...
 * ```SLURM_SIMULATION_TOOLKIT_REGRESS_DIR``` is the base directory beneath which simulation output directories and regression summary directories will be autogenerated. The default value in ```examples/example_master.rc``` is likely correct for most Compute Canada users.  
 * ```SLURM_SIMULATION_TOOLKIT_JOB_RC_PATH``` is the path to an RC file which contains default SLURM job parameters. Since these parameters can be overridden from the command-line, creating your own RC is not required.
 * ```SLURM_SIMULATION_TOOLKIT_GET_CLUSTER``` points to a script that outputs the name of the local cluster. The default value in ```example_master.rc``` is likely correct for Compute Canada users. 
