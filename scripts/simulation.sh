...
output_dir=${regress_dir}/mine/mine_${datetime_suffix}
...
simulation_options+=" --num_proc_per_gpu ${num_proc_per_gpu} --cmd sbatch -e ${export} --account ${account}"
if [[ ${email} == yes ]]; then
...
slurm.sh "${simulation_options}" "${sbatch_script_path}" |& tee -a ${prolog_file}
