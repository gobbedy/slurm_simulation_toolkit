...
regress_dir=${SLURM_SIMULATION_REGRESS_DIR}
...
output_dir=${regress_dir}/test/test_${datetime_suffix}_$(openssl rand -hex 4)
...
source_code_dir=$(dirname ${SLURM_SIMULATION_BASE_SCRIPT})
cp -rp ${source_code_dir}/* ${output_dir}

# full path to copied sbatch script
sbatch_script_path="${SLURM_SIMULATION_TOOLKIT_HOME}/scripts/simulation.sbatch"

# full path of base script
script_basename=$(basename ${SLURM_SIMULATION_BASE_SCRIPT})
base_script_path="${output_dir}/${script_basename}"

# prepare arguments to job script (slurm.sh)
export="script_path=\"${base_script_path}\",output_dir=\"${output_dir}\",ALL"
