...
OPTIONS
  ...
  --job_name JOB_NAME
                          JOB_NAME is the name of the SLURM job. This will be used in the autogenerated prolog and
                          simulation logfiles.
  ...
  --singleton
                          ...
                          for example a job named JOB_NAME is already running, this job will wait for that job to
                          finish before starting. Similarly, if this job is running, any future job named JOB_NAME and
                          having used the --singleton switch will wait for this job to finish.
