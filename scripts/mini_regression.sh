...
OPTIONS
  ...
  --blocking_job_manifest MANIFEST
                          ...
                          are launched. MANIFEST must contain exactly (SIMS/PROCS) job IDs separated by newlines.
  ...
  --job_name JOB_NAME
                          JOB_NAME is name of SLURM name. This will be used in the autogenerated output directory name.
                          If MAX is is provided (see --max_job_in_parallel) an index ranging from 0 to (MAX - 1) will
  ...
  --num_proc_per_gpu PROCS
                          ...
                          Default is 1 (process per resource).
...
regression_dir=${parent_dir}/regressions/${regression_name}

# Create names of files that will contain summary information about regression
regression_command_file=${regression_dir}/regression_command.txt
regression_logname_file=${regression_dir}/log_manifest.txt
regression_job_numbers_file=${regression_dir}/job_manifest.txt
